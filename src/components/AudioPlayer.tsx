...
import { getChapter, BIBLE_BOOKS, FEATURED_TRANSLATIONS, type Verse } from '../utils/bibleApi';
...
export const AudioPlayer: React.FC<AudioPlayerProps> = ({ translation, bookId, chapter, onClose }) => {
  ...
  useEffect(() => {
    ...
    getChapter(translation, bookId, chapter).then((verses: Verse[]) => {
      if (!active) return;
      const text = verses.map((v: Verse) => v.text).join(' ');
      ...
      <div className="flex-1 flex items-center justify-between px-4 sm:px-6 max-w-4xl mx-auto w-full">
        <div className="flex-1 flex items-center min-w-0">
          <div className="truncate">
            <h4 className="font-display font-semibold text-text-primary truncate">{bookName} {chapter}</h4>
            <p className="text-xs text-text-muted uppercase tracking-wider">{translationName}</p>
          </div>
        </div>
        <div className="flex-1 flex items-center justify-center gap-4">
          <button onClick={togglePlay} disabled={isLoading} className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors shadow-sm ${isLoading ? 'bg-bg-secondary text-text-muted cursor-not-allowed' : 'bg-accent-gold text-white hover:bg-accent-brown'}`}>
            {isPlaying ? <Pause size={20} fill="currentColor" /> : <Play size={20} fill="currentColor" className="ml-1" />}
          </button>
        </div>
        <div className="flex-1 flex items-center justify-end gap-3">
          <button onClick={cycleSpeed} title="S'applique au prochain chapitre" className="text-xs font-mono font-medium text-text-secondary hover:text-text-primary w-10 text-right">{speed.toFixed(1)}x</button>
